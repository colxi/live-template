...
* @Last Modified time: 2018-07-22 02:02:07
*/
...
const ConfigInterface = {
    // Getters
    get binderPrefix(){ return Config.binderPrefix},
    ...
    set binderPrefix( value ){
        // value validation
        if( typeof value !== 'string' ) throw new Error('Config:binderPrefix : Value must be a String.');
        value = value.trim().toLowerCase();
        if( !value.length  ) throw new Error('Config:binderPrefix : Value can\'t be an empty String.');
        if( value.indexOf('-') !== -1 ) throw new Error('Config:binderPrefix : Value can\'t contain dashes ("-").');
        if( Util.stringHasSpaces( value ) ) throw new Error('Config:binderPrefix : Value can\'t contain spaces.');
        // done! accepted!
        Config.binderPrefix = value;
        return true;
    },
